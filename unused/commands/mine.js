...
module.exports = {
	...
	execute(message, args) {
		let objects = JSON.parse(fs.readFileSync('databases/objects.json')),
            ...
            player_location_str = players[userName]['location'].join(', '),
            player_inventory = players[userName]['inventory'],
            commaArgs = args.join(' ').split(', '),
            NotEnoughSpaceLeft = false,
            list_of_minerals_obtained = [],
            mineral_name_with_num,
			mineral_name_arg,
			tool_name_arg,
			mineral_name,
		...
		if (state_of_mineral != 0) {
			...
			if (new_state_of_mineral === 0) {
				locations[player_location_str]['layers'][mineral_name_with_num]['information']['minerals'].forEach((mineral) => {
