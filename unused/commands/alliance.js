...
module.exports = {
    ...
    isServerOnly: true,
    args: true,
    required_servers: ['698178798759444531'],
    required_roles: ['Players'],
    required_categories: ['You Room', 'You Rooms'],
    usages: '[name-with-no-spaces] [<@user1 id>] [<@user2 id>]... (Ping the players you want in your alliance. Do not ping yourself)',
	execute(message, args) {
        if (args.length <= 1) {
            return message.channel.send(`I need more arguments than that! \n\`${prefix}${this.name} ${this.usage}\``)
        }

        if (args.length > 5) {
            return message.channel.send(`You can't have an alliance with more than 4 players`)
        }
        ...
        for (let i = 1; i < args.length; i++) {
            if (!getUserFromMention(args[i]).roles.cache.some(r => r.name === 'Players')) {
                return message.channel.send(`You may only have players in your alliance!`)
            }
        }
