...
module.exports = {
	...
	execute(message, args) {
        let objects = JSON.parse(fs.readFileSync('databases/objects.json')),
            ...
            harvest_info = JSON.parse(fs.readFileSync('harvest info.json')),
            userName = message.guild.members.cache.get(message.author.id).displayName.toLowerCase(),
            currentLocationText = players[userName]['location'].join(', '),
            plantName = args.join(' ').toLowerCase(),
            items_harvested_message = [],
            rand_exp_value = Math.floor( Math.random() * 2 ),
            num_of_items_harvested = 0,
            player_inv = players[userName]['inventory'],
            inv_space_of_player = player_inv['space'];
