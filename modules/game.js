...
class Game {
	...
	async announceMessages(...messages) {
		for (let message of messages) {
			...
			console.log(message);
			await wait(MessageDelays.Normal);
		}
	}
	...
	startTrial(days_passed, message) {
		...
		if (
			...
			const start_trial_cmd = require(`../commands/starttrial.js`);
	...
	startTrialResults(days_passed, message) {
		...
		if (
			...
			const start_trial_results_cmd = require(`../commands/starttrialresults.js`);
	...
	startDay(days_passed, message) {
		...
		if (
			...
			const start_day_cmd = require(`../commands/startday.js`);
	...
	startNight(days_passed, message) {
		...
		if (
			...
			const start_night_cmd = require(`../commands/startnight.js`);
			start_night_cmd.execute(message);
		}
	}


	async addPlayerToGame(player_name, player_id, isFakeUser=false) {
		let player_member;
		...
		if ( this.Players && this.Players.get(player_name) ) {
			return await join_chat.send(`The name, **${player_name}**, already exists.`)
		}

		const validator_result = validator.validateName(player_name);
		if (validator_result !== true)
			return await join_chat.send(validator_result);
		...
		join_chat.send(`**${player_name}** added to the game.`);
		console.log(this.Players.getPlayers());
	}
	...
	async endGame() {
		...
		let resetgame = require("../commands/resetgame.js");
