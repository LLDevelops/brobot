	...
	async execute(message, args, isTest=false) {
		...
		if (global.Game.next_deaths.length <= 0) {
			...
			if (global.Game.timeout_counter >= max_timeout) {
				...
				game_announce_chnl.send(`There have been no deaths for **${global.Game.timeout_counter} days**. **${max_timeout - global.Game.timeout_counter}** more and the game ends in a draw.\n`);
				await wait(...wait_times["message_delay"]);
			}
		}
		else {
