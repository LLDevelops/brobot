...
module.exports = {
	...
	async execute(message, args) {
		...
		const staff_chnl = await getChannel(message.guild, "1031365761672937533");
		...
		const areArgsValid = function(args) {
			...
			if (player_count <= 0) {
				staff_chnl.send(`There's no players. Try running \`<startsignups\``);
				return false;
			}
			// Check if player count equal role amount
			else if (player_count != args.length) {
				staff_chnl.send(`You have ${args.length} roles and ${player_count} players.`)
				return false;
			}
		...
		const announceDay1 = async function() {
			...
			global.Game.announceMessages(
				...
				`> — You can avoid typing out a player's entire name by only typing the first few letters as BroBot will try to autocomplete the name for you.`,
			);
